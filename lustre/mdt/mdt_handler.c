...
static int fsoptions_to_upcall_path(char *identity_upcall, size_t size, char *options)
{
        char *equal, *comma;

        if ((options = strstr(options, PARAM_MDT PARAM_UPCALL)) != NULL) {
                     ...
                     struct lu_device_type *ldt, struct lustre_cfg *cfg)
{
        ...
        if (m->mdt_opts.mo_acl)
                fsoptions_to_upcall_path(identity_upcall, PATH_MAX, lsi->lsi_lmd->lmd_opts);
        else
